...
app.jinja_env.filters["group_engines_in_tab"] = (
    group_engines_in_tab  # pylint: disable=no-member
)
...
def render(template_name: str, **kwargs):
    ...
    if kwargs["endpoint"] == "index" or kwargs["endpoint"] == "results":
        about_section = infopage.INFO_PAGES.get_page(
            "about", current_locale
        ).html.split("<h2>")[1]
    ...
    kwargs["proxify"] = (
        morty_proxify if settings["result_proxy"]["url"] is not None else None
    )
...
def preferences():
    ...
    # render preferences
    image_proxy = sxng_request.preferences.get_value(
        "image_proxy"
    )  # pylint: disable=redefined-outer-name
    disabled_engines = sxng_request.preferences.engines.get_disabled()
    ...
    checker_results = (
        ...
        if checker_results["status"] == "ok" and "engines" in checker_results
        else {}
    )
    for _, e in filtered_engines.items():
...
def page_not_found(_e):
    return render("404.html", status=404), 404
...
def init():
    ...
    searx.search.initialize(
        enable_checker=True, check_network=True, enable_metrics=metrics
    )
